...
$prod->load($product->product_id);
$images = $prod->getImages();
$count = count($images);
$extraFields = $prod->getExtraFields();
?>
...
<div class="product productitem_<?php print $product->product_id?>">
    ...
    <div class="images clearfix">
        <?php foreach ($images as $image) { ?>
                <a href="<?php print $product->product_link?>"><span class="object_image" style="background-image: url('/components/com_jshopping/files/img_products/<?php echo $image->image_full; ?>'); width: <?php echo 100/$count; ?>%;"></span></a>
		<?php } ?>
    </div>
    
	<div class="info">
	    <a href="/ob-ekty?manufacturer_id=<?php echo $product->manufacturer->id; ?>">
		<!--<a href="<?php echo SEFLink('index.php?option=com_jshopping&controller=manufacturer&task=view&manufacturer_id='.$product->manufacturer->id); ?>">-->
			<div class = "image" style="background-image: url('/components/com_jshopping/files/img_manufs/<?php echo $product->manufacturer->manufacturer_logo; ?>');">
		...
		<div class="desc">
		    <?php foreach($extraFields as $extraField) { ?>
				<p><span class="bold"><?php echo $extraField['name']; ?>: </span><?php echo $extraField['value']; ?></p>
			<?php } ?>
		</div>
