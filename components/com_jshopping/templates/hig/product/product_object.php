...
$cats = $cat->getAllCategories(1, 1, 'name');
foreach ($cats as $category) {
    if ($category->category_id == JRequest::getVar('category_id')) {
        $categoryName = $category->name;
    }
}
?>
<div class="jshop productfull" id="comjshop">
    <form class="clearfix" name="product" method="post" action="<?php print $this->action ?>"
        ...
        <div class="div_h1"><?php echo $categoryName; ?><?php if ($this->config->show_product_code) { ?> <span
                    class="jshop_code_prod">(<?php print _JSHOP_EAN ?>: <span
                        id="product_code"><?php print $this->product->getEan(); ?></span>)</span><?php } ?></div>
        ...
        <div class="joomshopping_full_block joomshopping_full_object">
            ...
            <div class="row-fluid jshop productfull_blocks clearfix">

                <div class="productfull_images productfull_block">
                    ...
                    <div class="span8 jshop_img_description">
                                    ...
                                    <a class="lightbox"
                                       ...
                                       title="<?php print htmlspecialchars($image->_title) ?>">
										<img class="jshop_img_thumb"
                                             src="<?php print $this->image_product_path ?>/<?php print $image->image_thumb ?>"
                                             alt="<?php print htmlspecialchars($image->_title) ?>"
                                             title="<?php print htmlspecialchars($image->_title) ?>"/>
									</a>
                                <?php } ?>
                        ...
                        <?php if (count($this->videos)) { ?>
                            <?php foreach ($this->videos as $k => $video) { ?>
                                <?php if ($video->video_code) { ?>
                                    <a href="#" id="video_<?php print $k ?>"
                                       onclick="showVideoCode(this.id);return false;"><img class="jshop_video_thumb"
                                                                                           src="<?php print $this->video_image_preview_path . "/";
                                                                                           if ($video->video_preview) print $video->video_preview; else print 'video.gif' ?>"
                                                                                           alt="video"/></a>
                                <?php } else { ?>
                                    <a href="<?php print $this->video_product_path ?>/<?php print $video->video_name ?>"
                                       id="video_<?php print $k ?>"
                                       onclick="showVideo(this.id, '<?php print $this->config->video_product_width; ?>', '<?php print $this->config->video_product_height; ?>'); return false;"><img
                                                class="jshop_video_thumb"
                                                src="<?php print $this->video_image_preview_path . "/";
                                                if ($video->video_preview) print $video->video_preview; else print 'video.gif' ?>"
                                                alt="video"/></a>
                                <?php } ?>
                ...
                <div class="productfull_short_description productfull_block">
                    <div class="short_description">
                        <?php print $this->product->short_description; ?>
                    </div>
                </div>

                <div class="productfull_block">
                    <?php jimport('joomla.application.module.helper');
                    $module = JModuleHelper::getModules('joomshopping_order');
                    $attribs['style'] = 'xhtml';
                    echo JModuleHelper::renderModule($module[0], $attribs); ?>
                </div>
